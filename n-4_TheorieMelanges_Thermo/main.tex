...
L'utilisation du potentiel chimique dans le cadre des mélanges permet de faire le lien entre les interactions des constituants chimiques et la forme des diagrammes de phase. Ce lien est extrêmment important pour faire le lien entre modèle et expérience, structure et propriété.

De plus, nous observerons que des modèles assez élémentaire permette de rendre compte qualitativement ou quantitativement des phénomènes thermiques des mélanges : coexistence de phase solide et liquide, démixion des solides en un mélange hétérogène$\ldots$
...
\begin{align*}
\gamma_i^\star > 1 &\implies \text{les interactions solvant-soluté destabilisent le solvant} \\
...
Dans la limite d'une solution infiniment diluée, on doit retrouver une activité proportionelle à la fraction molaire :
\begin{align*}
\lim_{x_i \to 0} \gamma_i^\infty (T, P, x_i) = 1
\end{align*}

Si le constituant $i$ est largement majoritaire, alors $a^\infty_i \to x_i$, on retrouve donc une activité proportionelle à la quantité de matière.
...
\subfloat[\label{fig:Chp4-01b}]{\includegraphics[height=4.0cm]{figures/Fig01b}}
\hspace{0.5cm}\subfloat[\label{fig:Chp4-01c}]{\includegraphics[height=4.0cm]{figures/Fig01c}}
...
\subsection{Idéalité}
Dans le cas où les interactions \ce{A-A} et \ce{B-B} sont équivalentes aux interactons \ce{A-B}, les c{\oe}fficients d'activités valent $1$ à toute composition.

\definition{
Dans un mélange idéal, le mélange est décrit par les égalités suivantes : 
\begin{align*}
\Delta H_m &= 0 \\ 
\Delta S_m &= - n^s R \left[ (1-x) \ln (1-x) + x \ln (x) \right] 
\end{align*}
...
\definition{Une solution régulière est un système où l'enthalpie de mélange est décrite sous la forme :
\begin{align*}
\Delta H_m = J_{AB} x_A \cdot x_B
\end{align*}

Le terme $J_{AB}$ est un paramètre mesurant les interactions entre les constituants \ce{A} et \ce{B} voisins.

Les interactions seront notées sous forme adimentionnée sous la forme : 
\begin{align*}
\Delta H_m = \underbrace{RT \chi_m}_{J_{AB}} x_A \cdot x_B
\end{align*}

Dans un tel modèle, le c{\oe}fficient d'activité dans l'ERCP s'écrive sous la forme : 
\begin{align*}
\ln \gamma_A^\star (x, T) = \chi_m (T) \cdot x^2 \\ 
\ln \gamma_B^\star (x, T) = \chi_m (T) \cdot (1-x)^2
\end{align*}
}
...
Il est rare que l'on soit capabl de modéliser complètement la fonction enthalpie libre. Cependant, son expression générale, sous la forme :
\begin{align*}
G &= n_A \mu_A^\star + n_B \mu_B^\star + \Delta G_m \\
&= n_A \mu_A + n_B \mu_B
\end{align*}

est toujours vraie et constitue une base intéressante à l'interprétation des phénomènes tel que la coexistence de deux phases : solide/liquide, solide/solide, la démixion à basse température$\ldots$

Dans toute cette section, on considérera que un mélange de deux constituants A et B à l'état solide et on se limitera à l'étude du phénomène de miscibilité partielle (= démixion). Une demixion partielle peut se modéliser sous la forme : 
...
\subsection{Lecture qualitative des potentiels chimiques}

Le potentiel chimique de chacun des constituants dans une phase donnée est une grandeur directement accessible en traçant la tangente à l'enthalpie libre molaire, d'équaton : 
\begin{align*}
...
\definition{Dans une phase donnée, le tracé de la tangente de la fonction enthalpie libre molaire n $x_0$ permet de lire le potentiel chimique du constituant \ce{A} (resp. \ce{B}) dans cette phase à la composition $x_0$.}

\subsection{Comparaison d'un système hétérogène et d'un système homogène}

Les courbes d'enthalpies libre nous permettent en général d'interpréter l'existence, ou non, de une ou plusieurs phases à un composition donnée.

Lorsque la courbe d'une phase homogène est connue, on est peut-être emmené la comparer à la courbe d'un mélange hétérogène.
...
\end{minipage}\hfill\begin{minipage}{0.38\textwidth}
\captionof{figure}{Evolution de l'enthalpie libre de mélange dans un solide décrit par un modèle de solution régulière}
[$4$ cordes représentant $4$ mélanges hétérogènes sont moins stables que le mélange homogène]
\label{fig:05b}
\end{minipage}
\end{center}
...
&= \bar{G} (T, P, x_B^\beta) x^\beta + \bar{G} (T, P, x_B^\alpha) \underbrace{(1 - x^\beta)}_{x^\alpha}
\end{align*}
...
Un système évoluant vers le mélange hétérogène le plus stable se traduit graphiquement par la corde de plus basse enthalpie.

Lorsque l'on minimise l'enthalpie de la corde, on trouve que la solution mathématique est une tangente commune au point de composition $x_B^\alpha$ et $x_B^\beta$ (assez intuitif du point de vu géométrique).
...
De plus, nous suposerons que le liquide et le solide sont incompressibles, ce qui implique que le volume molaire du solide ne dépend pas de la pression. Par conséquent, le potentiel chimique des constituants peuvent se réduire à : 
\begin{align*}
...
\begin{align*}
\mu_A^l (T, x_A^l) &=  \mu_A^s (T, x_A^s) \\
\mu_B^l (T, x_Bé^l) &=  \mu_B^s (T, x_B^s)
\end{align*}
\end{enumerate}
...
Les équations du solidus sont, dans un mélange hétérogène à miscibilité nulle : 
\begin{align*}
...
Comme le montre la Fig. \ref{fig:03c}, le rapport de la température de fusion des corps purs suit exactemeent les mêmes tendances que le rapport d'enthalpies de fusion.
